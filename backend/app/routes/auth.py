...
def login():
    ...
    log_audit(action="Login Success", user_id=user.user_id, details=f"User logged in successfully with role: {user.role}")
...
def update_profile():
    ...
    email_conflict = User.query.filter(User.email == email, User.user_id != current_user_id).first()
    if email_conflict:
    ...
    log_audit(action="Profile Updated", details=f"User {user.name} ({user.employee_id}) updated profile details")
...
def change_password():
    ...
    log_audit(action="Password Changed", details=f"User {user.name} ({user.employee_id}) changed their password")
...
def first_login_change_password():
    ...
    log_audit(action="First Login Completed", user_id=user.user_id, details=f"User {user.name} ({user.employee_id}) completed first login security setup")
    log_audit(action="Password Changed", user_id=user.user_id, details=f"User {user.name} ({user.employee_id}) changed temporary password successfully")
